...
main(int argc, const char **argv)
{
    ...
    sehle_material_dns_set_has_colors(mat, 1);
    sehle_material_dns_set_transparent(mat, 1, 0);
    SehleTexture2D *tex = sehle_engine_get_standard_texture(engine, SEHLE_TEXTURE_NOISE_16X16);
     ...
     */
    SehleDirectionalLightInstance light;
    az_instance_init(&light, SEHLE_TYPE_DIRECTIONAL_LIGHT);
    sehle_directional_light_setup(&light, engine, 0, 0);
    light.light_inst.ambient = EleaColor4fBlack;
    light.light_inst.diffuse = EleaColor4fWhite;
    EleaVec3f light_pos = {200, 400, 300};
    elea_mat3x4f_set_look_at(&light.light_inst.l2w, &light_pos, &EleaVec3f0, &EleaVec3fZ);
    sehle_render_context_add_light(&ctx, &light.light_inst);
    ...
    EleaMat4x4f proj;
    elea_mat4x4f_set_frustum_fov(&proj, 0.01f, (float) width / height, 0.01f, 10.0f);
    ...
    while (alive) {
        ...
        /* Display renderable */
        sehle_render_context_display_frame(&ctx, SEHLE_STATIC_MESH_RENDERABLE_IMPLEMENTATION, &mesh.renderable_inst, 1, SEHLE_STAGE_FORWARD | SEHLE_STAGE_TRANSPARENT);
        ...
        sehle_render_context_render (&ctx, SEHLE_STAGE_TRANSPARENT, SEHLE_RENDER_TRANSPARENT, 1, 0);
        /* Clean up context */
